<?php 
	...
	{
		function add($mahoadon, $ngaydat, $trigia,  $magiamgia='', $tenkhachhang, $sodienthoai, $diachi, $email,$trangthaidonhang=0, $ngaytao, $trangthai=1, $ngaycapnhat)
		{
			try 
			{
		...
		function listdata($orderby, $limit)
		{
			try 
			{
			  $sql = "select * from xshop_hoadon join xshop_trangthaidonhang on bill_trangthaidonhang = stb_ma order by ? desc limit ? ";
				$this->setQuery($sql);
		...
		function edit_trangthai2($value, $mahoadon)
		{
			...
			try 
			{
				$sql = "update xshop_hoadon set bill_ngaycapnhat='$update', bill_trangthai=? where bill_mahoadon=?";
				$this->setQuery($sql);
